...
public interface OrderTrackingRepository extends CrudRepository<OrderTracking,Long> {
    ...
    @Query(value = "SELECT * FROM order_tracking o WHERE o.seller_id = :sellerId AND o.status =:status ORDER BY o.created_at DESC OFFSET 5 * :minus LIMIT 5 * :count - 5 * :minus", nativeQuery = true)
